...
// 수식을 분석(필요한 경우 변환), 수식의 타입 계산하여 노드에 저장하고 리턴
...
// 함수 호출 명령문을 분석 중 파라미터의 개수와 타입이 일치하는지를 분석
...
// 명령문들을 분석, 각 명령문들에 나타난 지역변수들의 크기를 계산하여 그들 중 최대값을 리턴
...
// 선언문에 나타난 각 지역 변수의 주소를 설정, 크기 계산 후 리턴
...
void semantic_error(int i, int ll, char *s) {
    ...
    switch(i) {
        ...
        case 39:
            ...
            break;
        case 40:
            printf("illegal expression type in relational opeartion\n");
            break;
        case 41:
...
/*
// 초기화 수식을 나타내는 신택스 트리를 분석, 타입과 값을 계산/리턴
